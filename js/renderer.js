...
class Renderer {
    ...
    resize() {
        ...
        const scaleFactor = Math.min(this.W / 1400, this.H / 900, 1.2);
    ...
    _drawOpponentHands(game) {
        ...
        if (game.hands[2]) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const ty = 25;
                if (isReveal) {
                    this.drawTile(tx, ty, game.hands[2].concealed[i], false, true);
                } else {
        ...
        if (game.hands[1]) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (isReveal) {
                    ...
                    this.drawTileBack(tx, ty, true, true);
                }
            }
            this.ctx.save();
            this.ctx.fillStyle = '#AADDAA';
    ...
    _drawDiscardPiles(game) {
        ...
        this._drawDiscardGrid(game.hands[1].discards, cx + 100, cy, smallW, smallH, gap, cols, 'left');
        // Left player discards
        this._drawDiscardGrid(game.hands[3].discards, cx - 100, cy, smallW, smallH, gap, cols, 'right');
    }
    ...
    _drawExposedArea(game) {
        for (let p = 0; p < 4; p++) {
            ...
            if (p === 0 || p === 2) { // Horizontal row (Bottom / Top)
                ...
                // For simplified horizontal, we'll keep flowers in one line if <= 8, else 2 lines
                ...
                startY = (p === 0)
                    ? this.H - this.TILE_H - smallH - 30  // Bottom
                    : 25 + this.TILE_H * 0.7 + 15;        // Top
            } else { // Vertical stack (Right / Left)
                // Each flower row (max 4 tiles wide) followed by each meld row
