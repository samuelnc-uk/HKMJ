...
class UI {
    ...
    drawMenu() {
        ...
        panelGrad.addColorStop(0, 'rgba(20,60,30,0.85)');
        panelGrad.addColorStop(1, 'rgba(10,40,20,0.9)');
        ...
        const diffW = isPortrait ? (panelW - 100) / 3 : 95;
        for (let i = 0; i < 3; i++) {
            const bx = px + (isPortrait ? 85 : 130) + i * (diffW + 10);
            const bw = diffW;
            const bh = 34;
        ...
        optY += isPortrait ? 65 : 55;

        // ----- Min Fan -----
        ctx.font = 'bold 16px "Noto Sans TC", sans-serif';
        ctx.fillStyle = '#E8D8A0';
        ctx.textAlign = 'left';
        ctx.fillText('起糊:', px + 40, optY);

        const fanOptions = [1, 3];
        const fanNames = ['1番', '3番'];
        for (let i = 0; i < 2; i++) {
            const bx = px + (isPortrait ? 85 : 180) + i * 110;
            const bw = 100;
            const bh = 34;
        ...
        optY += 15;
        const themeCols = isPortrait ? 4 : 5;
        const themeW = isPortrait ? (panelW - 80) / themeCols : 78;
        for (let i = 0; i < 10; i++) {
            const col = i % themeCols;
            const row = Math.floor(i / themeCols);
            const bx = px + 40 + col * (themeW + 8);
        ...
        optY += isPortrait ? (Math.ceil(10 / themeCols) * 42 + 10) : 85;
        ...
        const charW = isPortrait ? (panelW - 60) / 2 : 100;
        const charH = isPortrait ? 90 : 100;
        const charGap = isPortrait ? 10 : 15;

        let charY = optY + 15;

        for (let i = 0; i < 4; i++) {
            const col = isPortrait ? (i % 2) : i;
            const row = isPortrait ? Math.floor(i / 2) : 0;

            const bx = isPortrait
                ? px + 25 + col * (charW + charGap)
                : px + (panelW - (4 * charW + 3 * charGap)) / 2 + i * (charW + charGap);
            const by = charY + row * (charH + (isPortrait ? 25 : 0));
            ...
            if (this._charStrip.complete && this._charStrip.naturalWidth > 0) {
                ...
                const portraitSize = isPortrait ? 60 : 72;
                const dx = bx + (charW - portraitSize) / 2;
                const dy = by + (charH - portraitSize) / 2 - 5;

                ctx.save();
                ctx.beginPath();
                ctx.arc(bx + charW / 2, by + charH / 2 - 5, portraitSize / 2 - 2, 0, Math.PI * 2);
                ctx.clip();
    ...
    _drawClaimButtons(game) {
        ...
        const btnW = isPortrait ? 75 : 90;
        const btnH = isPortrait ? 55 : 42; // Taller for touch
        const btnGap = isPortrait ? 8 : 10;
        ...
        for (const act of allActs) {
            this._drawActionBtn(startX, by, btnW, btnH, allLabels[act], allColors[act]);
            this.buttons.push({ x: startX, y: by, w: btnW, h: btnH, action: 'claim_' + act });
            startX += btnW + btnGap;
        }
    }
