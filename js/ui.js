...
class UI {
    ...
    drawRoundEnd(game) {
        ...
        if (game.winner >= 0) {
            // Display winner slice from win.png (2x2 grid, each 1376x768)
            // Player 0: TL, Player 1: TR, Player 2: BL, Player 3: BR
            const sw = 1376;
            const sh = 768;
            const sx = (game.winner % 2) * sw;
            const sy = Math.floor(game.winner / 2) * sh;
